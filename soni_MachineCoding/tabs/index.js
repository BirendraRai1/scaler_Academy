...
const buildTabs = (activeTabNum=1)=>{
    tabs.forEach(({id,title,content})=>{
        ...
        contentDiv.setAttribute("class",id===activeTabNum?'content active':'content')
...
const onTabClick = (event)=>{
    ...
    if(clickedTab.classList.contains("tab") && clickedTab !==activeTab){
        ...
        contents.forEach((content)=>{
            const contenTabId = content.getAttribute("data-tab")
            content.classList.toggle("active",contenTabId===tabId)
        })
    }
}
